...
export class ScannerService {
  ...
  state: ScanState = {
    ...
    warm: false, consecutiveFailures: 0, bootedAt: Date.now(),
  };
  ...
  async scan(opts: { limit?: number; force?: boolean } = {}): Promise<ScanState> {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(limit, sets.length); i++) {
        batch.push(sets[(this.cursor + i) % sets.length]);
      }
   ...
   *
   * Render's free tier restarts and spins down instances at will, so this assumes a cold,
   * empty process every boot: it re-fetches everything from Warframe.market, never relying
   * on prior DB state. Failures back off exponentially instead of hammering the API, and the
   * loop self-heals once upstream recovers.
   */
