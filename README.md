...
**The database is optional.** It stores history, the watchlist and saved settings only — never
live market data. On an ephemeral host it is recreated empty at every boot, and all access goes
through `withDb()` so the app degrades gracefully rather than erroring when it is unavailable.
...
61 unit tests cover single-part sets, multi-part sets, quantities > 1, missing market orders, zero and
...
Render resilience (database degradation, `$PORT`/`0.0.0.0` binding, deployment config).
